...
IGNORED_DIRECTORY_NAMES = {"bin", "obj", ".git", ".idea", ".vscode", "__pycache__"}


def should_check(directory: Path) -> bool:
    if directory.name in IGNORED_DIRECTORY_NAMES:
        return False
    return any(scope in directory.parts for scope in SCOPES)
